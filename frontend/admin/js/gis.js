    ...
    try {
      ...
      const updateDense1mContours = () => {
        try {
          ...
          // Công thức khoảng cách đường đồng mức theo tiêu chuẩn bản vẽ địa hình CAD:
          // Zoom 16.0 (Xem toàn cảnh nông trại): Interval = 2.5m
          const currentZoom = map.getZoom();
          let interval = 2.5 - (currentZoom - 16.0) * 0.5;
          interval = Math.max(1.0, Math.min(5.0, interval));
          ...
          // Lọc mịn lưới DEM bằng Gaussian Blur 2 pass để loại bỏ đường gấp khúc / vòng lặp nhiễu
          const grid = smoothGrid(rawGrid, 2);
